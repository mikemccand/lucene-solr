...
public final class WordDelimiterStage extends Stage {
  ...
  // we change the offset (sometimes!):
  ...
  private int[] wordPosNodes = new int[4];

  /**
   * Creates a new WordDelimiterStage
   *
   ...
   */
  public WordDelimiterStage(Stage in, byte[] charTypeTable, int configurationFlags, CharArraySet protWords) {
    super(in);
  ...
  /**
   * Creates a new WordDelimiterStage using {@link WordDelimiterIterator#DEFAULT_WORD_DELIM_TABLE}
   ...
   * @param protWords If not null is the set of tokens to protect from being delimited
   */
  public WordDelimiterStage(Stage in, int configurationFlags, CharArraySet protWords) {
  ...
  private void bufferWordParts() throws IOException {
    ...
    wordPosNodes = ArrayUtil.grow(wordPosNodes, wordPos+1);
    wordPosNodes[0] = arcAttIn.from();
    System.out.println("wordPosNodes[0] = " + wordPosNodes[0]);
    wordPosNodes[wordPos] = arcAttIn.to();
    System.out.println("wordPosNodes[" + wordPos + "] = " + wordPosNodes[wordPos]);
    for(int i=1;i<wordPos;i++) {
      wordPosNodes[i] = newNode();
      System.out.println("wordPosNodes[" + i + "] = " + wordPosNodes[i]);
    }
  ...
  public boolean next() throws IOException {
    while (true) {
      if (bufferedLen == 0) {
        ...
        if (delAttIn.isDeleted()) {
          // don't process deleted tokens
          copyAllAtts();
          return true;
        }
        ...
        // word of simply delimiters; if we are not preserving original then mark the token deleted:
      ...
      if (bufferedPos < bufferedLen) {
        ...
        if (hasIllegalOffsets) {
          ...
          endOffset = offsetAttIn.endOffset();
        } else {
          startOffset = offsetAttIn.startOffset() + startPart;
          endOffset = offsetAttIn.startOffset() + endPart;
        }
        ...
        System.out.println("startNode=" + wordPosNodes[startPos] + " endNode=" + wordPosNodes[endPos]);
        arcAttOut.set(wordPosNodes[startPos], wordPosNodes[endPos]);
        System.out.println("WDS offsets: " + startOffset + "-" + endOffset);
        return true;
      }
  ...
  public String toString() {
    StringBuilder b = new StringBuilder();
    b.append("WordDelimiterStage(flags=");
